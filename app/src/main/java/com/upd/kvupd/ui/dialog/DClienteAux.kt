...
@AndroidEntryPoint
class DClienteAux : DialogFragment() {

    private var _bind: DialogClienteAuxBinding? = null
    private val arg: DClienteAuxArgs by navArgs()
    private val bind get() = _bind!!
    private val _tag by lazy { DClienteAux::class.java.simpleName }
        ...
        bind.btnMapa.setOnClickListener {
            if (PROCEDE == "Mapa") {
                ...
                findNavController().navigate(
                    DClienteAuxDirections.actionDClienteAuxToFMapa(arg.cliente)
                )
                dismiss()
            }
        }
