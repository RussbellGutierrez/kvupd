...
@AndroidEntryPoint
class OldFMapa : Fragment(), OnMapReadyCallback, OnMarkerClickListener,
    ...
    private val arg: OldFMapaArgs by navArgs()
    ...
    private lateinit var rutas: List<TableRuta>
    private lateinit var mclk: Marker
    private val _tag by lazy { OldFMapa::class.java.simpleName }
    ...
    override fun onMapReady(p0: GoogleMap) {
        p0.also {
            ...
            map.apply {
                ...
                setOnMarkerClickListener(this@OldFMapa)
                setOnInfoWindowClickListener(this@OldFMapa)
                setOnInfoWindowLongClickListener(this@OldFMapa)
    ...
    private fun searchList(list: List<DataCliente>) {
        ...
        findNavController().navigate(
            OldFMapaDirections.actionFMapaToDBuscar(nl.toTypedArray())
        )*/
    }
        ...
        when (dialog) {
            0 -> when (CONF.tipo) {
                "V" -> {
                    DESTINO_NAV = "mapa"
                    findNavController().navigate(
                        OldFMapaDirections.actionFMapaToFAlterno(cliente.id)//FMapaDirections.actionFMapaToBDObservacion(item)
                    )
                }
            ...
            1 -> findNavController().navigate(
                OldFMapaDirections.actionFMapaToDClienteAux(item)
            )
        }*/
    }
